...
### ⚠️ Но при открытии `.veg` эффект визуально не работает

Хост исправен, а результат для пользователя — нет, и это разные вещи.

Замер по цепочке, восстановленной из `project_big--buck-bunny_4x3-preview-reverse-fades-fx.veg`
(кадр 512×512 с резкой границей, среднее отклонение на пиксель по RGB):

| Что подаётся плагину | Трасса | Среднее отклонение |
|---|---|---|
| Слот как есть из `.veg` (параметров нет) | `Render -> status 0` | **2.6** — невидимо |
| Те же параметры, что в проекте (`HorizontalPixels`/`VerticalPixels` = 20) | `Render -> status 0` | **45.3** — явный блюр |

Причина: **OFX-блоб параметров из `.veg` не декодируется**. Слот приезжает с пустой картой
параметров, инстанс берёт объявленные плагином дефолты, а у Chroma Blur это `0` — то есть
радиус нулевой и рендер вырождается в округление цветового преобразования. Плагин отрабатывает
честно, просто ему нечего делать.

В файле эти значения лежат читаемо — UTF-16 имя параметра (`HorizontalPixels`) и следом
double'ы с кейфреймами, той же формы, что уже разобранное состояние legacy-эффектов. То есть
это работа по парсеру, а не очередной барьер хоста.

**Пока не сделано.** Практический итог: **видеоплагины из Vegas Pro не работают.** Цепочка
восстанавливается, плагины видны в каталоге и в окне Event FX, но картинки не дают. Эффект,
добавленный руками (Drag'n'Drop из панели Video FX + правка ползунков), рендерится
по-настоящему — там параметры есть.
