...
|------|----------|
| Size | 19096 bytes |
| MD5 | `309fbfafc93abd6cd62cb0f58b62e149` |
| Filesize field @0x10 | 19096 |
...
`D:\Devs\C++\OpenVegas\sample_ui\example_project_with_2_videos-one_compressed.veg`
...
|--------|------------|---|--------------------|--------------------------|
| `0x1980` | 2.455782340109 | **245.6%** | 2.452450 s | 6.022683 s |
| `0x31C8` | 2.455782340109 | **245.6%** | 2.452450 s | 6.022683 s |

Паттерн: `u64 ?`, `u64 length_ticks`, `u64 0`, **`f64 rate`**, `u64 0`.  
Единица времени ≈ **10 000 000 ticks/сек**. UI VEGAS: `rate × 100` → **245.6%** (с запятой в локали).
...
000010  98 4A 00 00 00 00 00 00 EF 29 C4 46 4A 90 D2 11   .J.......).FJ...
...
000070  00 00 00 00 00 00 00 00 03 00 00 00 A2 00 00 00   ................
```

## Рассуждения

- Ранний блоб (~3832 B) — props + путь + ProjectNotes; далее media pool и timeline.
- Time-stretch: **245.6%** ×2 (video+audio pair).
- Rate > 1 → **сжатие / ускорение** (velocity accordion плотнее, badge >100%).
- Один и тот же кусок медиа (~**6.02 s**) укладывается в событие **2.452 s** на таймлайне.
